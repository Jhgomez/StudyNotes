...
* **Event system/Input Events**: Knowing how to handle events inside a custom view can help you create very custom user experiences and that to me is the most important reason why you should know how they work and what you can do with them. Be aware that now a days most events occur in "touch mode", as in the past devices very commonly had phyisical keyboards, trackballs, and that is how the touch mode was born, while we explain the differences betwen using the trackball and the touch screen we will see how the focus concept changed. Pretty much, touch screen replaced both keyboards and trackballs. When trackballs were common in devices, developers had to handle focus very meticulously because using the trackball you had to navigate through the view hierarchy, for example you had a screen with two buttons, using the trackball you will select the second botton and then click it, usually the first button will have focus by default, but the focus had to be visual also(something like a highlight or some outline), but when devices started transitioning from trackball to touch screen they would usually have both, a trackball and a touch scree, so when the user was using the trackball and switch to using the touch screen, the device also switched to something called "touch mode", in here the most views don't receive visual focus(means "focusableInTouchMode" is false) the same way they did with the trackball, but there is still views that need to be focusable like text edit/text input views, the focus is also visual here(means "focusableInTouchMode" is true). Now a days, pretty much all devices are touchscreen, that means touch mode is pretty much always on, so now when an activity is rendered, the top most view receives focus(no visual focus though, means "focusableInTouchMode" is false), if there is an edit text view, you can request focus programatically or just tap it and receive focus(edit texts should have visual focus, means "focusableInTouchMode" is true), there are other views that should be focusable, but not visually focusable, that means "focusableInTouchMode" is false but "isFocusable" is true, this is what happens with clickable buttons, if the button is not focusable in theory it won't be able to consume a click event. So we can see that the "focus" concept is very important in how events work. Once the keyboards and trackballs dissapear, we were only left with touch screen devices, usually touch screen devices are said to only have software input methods, and when having keyboards or trackballs, you will be able to track hardware input methods events, a very specific example is the view's interface `onKeyListener` and the activity's methods `onKeyDown` and `onKeyUp`, these three scenarios are only called with hardware key events, if you want to react to events in the IME(input method editor) AKA "soft keyboard", then you have to combine two approaches, for letters/characters/symbols/numbers use a `TextWatcher` object and pass it to an edit text `addTextChangedListener`, and for clicks on IME actions like "next", "done", set the edit text's `setOnEditorActionListener` method and compare againts things like `EditorInfo.IME_ACTION_DONE`. There is two ways you can handle events, using either **event handlers** or **event listeners**, events behavior can be defined/customized by either overriding it's methods in a custom view, for example to handle a click which is actually a series of of touch events, you can override `onTouchEvent()` method, or you could override an event listener setter method(`set`.. `OnClick`, `OnLongClick`, `OnFocusChange`, `onKey`, `onTouch`, `onCreateContextMenu`, ...`Listener`) these two options might be what you need when creating a custom view, however most commonly you will define event behavior by instantiating event listener interfaces and pasisng that instance to the right event listener method, for example instantiate an `OnClickListener` and pass that to `setOnClickListener`, which is very commonly done in the `onCreate` method of an activity, you could implement the interface in your activity or fragment, override the respective method and pass the fragment instance to the listener's setter method, this apporach avoids the overhead of instantiating interfaces objects. Knowing how to use input events we can manage our custom views scroll behaviour and customize things like panning, fling and drag(scrolling), using events we also can enable zooming in a custom view. An important API in detecting scroll, fling, and other gestures is the `Scroller` and `OverScroller`
