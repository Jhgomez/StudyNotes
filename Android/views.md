...
* **Custom Views**: When creating a custom view, if you want to use 3D graphics, extend `SurfaceView` instead of `View`/`ViewGroup` and draw from a separate thread. See the `GLSurfaceViewActivity`( implementation of `SurfaceView` that uses the dedicated surface for displaying OpenGL rendering), otherwise if you want to do 2D graphics, or just a styled view, then you may need to override the `onDraw()` method which receives and argument of type `Canvas`. Other very important method is 
