...
resources like a response from a server. We also have UI logic, which acts over UI state, like changing the color or size of a text based on some value or screen state. Again UI state can be
of two types, but we also said they both can live in the same state holder(if UI element state needs to interact with business logic), either way, we can say UI state is application data
