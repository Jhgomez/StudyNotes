...
#### FragmentContainerView vs Fragment(XML tag) (How to find fragments)
Both views are designed to host fragment instances, this means you can instantiate a fragment and have the instance of the fragment displayed inside one of these view groups, actually it seems like only the former is an instance of a view group and I making this assumption since I was not able to review the `Fragment` source code for some reason(my impression is that it is a special component and not exactly a view group, and also it is curious that the same component seems to be used when declaring a fragment inside a navigation graph, `navigation` tag, that is why it seems kind of special case). Theres is two ways to display fragments inside these two containers, you can do it manually or "automatically", doing it manually implies using the low level APIs yourself(`FragmentManager`), and automatic management is accomplished using jetpack's navigation component which under the hood also uses fragment manager but introduces high level APIs like `NavHost`, `NavCOntroller`, and `NavGraph`. Manually displaying a fragment inside one of these components is accomplished with the help of the corresponding fragment manager, which means that if a container is declared inside an activity then the activity's support `FragmentManager` is in charge to load the fragment that lives inside the container(`FragmentContainerView` or `Fragment`), if you intend to use the same container to display a series of fragments, like a login flow that displays different screens, transactions like `add`, `replace` will let you attach a tag to the fragment instance and this is an important detail since this tag is used by the `FragmentManager` to locate a fragment, but this is not the only way, you can also identify it by an Id, but you can not specify an Id to a fragment instance, so in this context it is best to identify a fragment by a tag because you can locate several fragments using the tag each one was introduced with at the time a transaction was processed by the fragment manager, but we also need to understand the Id property, in this context the Id we are referring to is the id of the container, if we declared that in a XML then is the `android:id` property, and if we created the container programatically then we can set the id programatically, but the point here is that the id is not the fragment that is being displayed but instead is the id of the container so if I would search the fragment by the id of the container then the fragment manager would return the latest added fragment instance and with this approach we would not be able to find other fragments in the backstack. Automatically displaying a fragment using the navigation component implies that we are using the `NavHost` which is an implementation of a fragment and inside this fragment there it exists a `NavController` which can handle a series of fragments that are declared inside a `navigation` resource(`NavGraph`) which declares `fragment`s(the same component mentioned in this pargraph earlier giving us the posibility to add an id to each fragment) and inside these `fragment`s we can declare parameters which are called navigation arguments, and `action`s which indicate how a fragment can navigate to another fragment, so in this context we can see a few things like, if our app is a single activity app then activity's support `FragmentManager` loads the `NavHostFragment` inside the container, which again is a `Fragment` instance/implementation, and in turn the `NavController` which lives inside the `NavHostFragment` uses the `NavHostFragment`'s child `FragmentManager` to start navigating through the current `NavGraph`, So for example if I wanted to find the first fragment of the first navigation graph after starting an app from the main activity then I would access the actvity's support `FragmentManager` and find the navhost fragment by the id of the container(in this case we wouldn't be able to find it by a tag since we didn't start the transaction manually we weren't able to set the tag of this fragment) with the following line `getSupportFragmentManager().findFragmentById(R.id.container_id)` the fragment returned by this call can be used in different ways in order to find an specific fragment in the backstack since it can be either casted to a regular `Fragment` instance or a `NavHostFragment` instance, if we cast it to a `Fragment` we can only access it's child `FragmentManager` and find the specific fragment by the id, if we cast it to a `NavHostFragment` we can also access the child `FragmentManager` directly and find the fragment by id. Now be aware that we can have the container automatically create, load and display an instance of the desired fragment by specificying its `android:name` attribute which is the fully qualified class name of the fragment we want it to display, when using nav component it is the `NavHostFragment` qualified name and also we can set up an initial navgraph, with `app:navGraph`, additionally we can should set up the `app:defaultNavHost` property, otherwise we need to start a transaction and commit it all manually in either use case of navigation options. There is another thing we may want to do, maybe we don't need to get the instance of a fragment but maybe just verify if a screen is in the backstack which inherently means it was previously visited and we still can return to it if we wanted, or what is the current fragment being displayed and here we can use two APIs which again are separated by low level and high level using a `FragmentManager` we can verify if a fragment instance exists, if doing it all manually we have the `FragmentManager.BackStackEntry` API which represents a transaction that already happened, and in the navigation component context we could use the `NavDestionation` API which represents a screen/fragment we could go to, have already went to, or the screen we currently are. In either case these two APIs can verify against an id property which are not defined nor generated the same way. Manually the id of a `BackStackEntry` is defined when starting a transaction using `addToBackStack(String name)` but is actually transformed into an `int` value and returned by the `commit()` method when committing a transaction, this means we could store the `int` returned by the `commit` method to identify the entry later, but we would need to compare the id property of all entries one by one to find it since we only have two methods to interact with the `BackStackEntry` without modifiying it from a `FragmentManager`, `getBackStackEntryAt(int index)` and `getBackStackEntryCount()`. If using the navigation component we can use the `NavDestination` API but the only convenience method we can find lives in `NavController` objects and is `getCurrentDestination()` which lets us access to the id property of the returned `NavDestionation` object returned by it and this id represents the value generated from the `android:id` property that lives inside a `fragment` tag inside the `navigation` tag(`NavGraph`) so this means that in automatic navigation it is easier to know if the current fragment being displayed is the one we want with code like `navControllerInstance.getCurrentDestination().getId() == R.id.name_of_framgnet_inside_navgraph` 
