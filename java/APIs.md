...
FxLauncher is a library for JavaFx that lets you upload files to a remote computer(server) using some of the bellow Java APIs and by using the Linux command/utility called secure copy `scp` to copy files to a remote server usings SSH, it creates a manifest in a "xml" file which is also updloaded also, this XML file is "packed" as a resources in the final product(client/desktop application) and is used to compare against the XML that is uploaded to our server, if it detects changes it downloads the JAR files that has changed and uses java `URLClassLoader` to load the classes contained in those JAR files to the JVM at runtime
...
There is different type of class loaders, the first "group" use a hierarchical order from child to parent, System or application class loader, Platform class loader, Bootstrap class loader, first it goest to application which delegates search to platform which delegates to the boostrap class loader(AKA primordial/main class loader), if parent can not find class it return to child and if the last child can not find the class it throws an execption. Boostrap loads JDK internal classes, platform load standard core Java classes, system loads application-level classes, it loads files found in the `-classpath`/`cp`(command line option) environment variables. However, there is a problem, resources loaded by parent class loaders are visible to the child class loaders, but not vice versa and, in some scenarios, parent class loaders might need to access classes present on the classpath of child class loaders but they can't, at least on their own, and this is only possible by using the "other type" of classs loaders called "context class loaders", using context loaders We can explicitly specify the class loader while loading a class dynamically. However, we should note that if we load the same class from different types of class loaders, these will be seen as different resources by the JVM. Each thread in Java has an associated context class loader, we can access/modify the context class loader for a thread. each thread context loader is set at the time of the creation of a thread. If not set explicitly, it defaults to the context class loader of the parent thread. Context class loaders also follow the hierarchy model. The root class loader, in this case, is the context class loader of the primordial thread. A primordial thread is the initial thread created by the operating system. By changin the context loader of a thread we’re basically switching the context for loading resources. Instead of searching at the current classpath, we fetch a new class loader pointing to a different classpath. This is particularly helpful if we want to load resources from a third-party module or if we are working in an environment with different class namespaces. Hence, we can use a child thread’s context class loader and can locate a resources present at the child class loader level from a parent class loader. We should always reset the context class loader property back to the original class loader to avoid any future discrepancies. Basically we should temproarelly change the context class loader just to load classes in a differente classpath. Commonly used in modular systems like OSGi or Java EE containers, where different modules or plugins may have their own class loaders.

Be aware, the "platform class loader" is the equivalent of what used to known as the "extension class loader". One motivation for renaming it is that the extension mechanism has been removed.
...
### How FXLauncher is using this API?
The intent of using this API in this library is to provide the chore functionality to load new jars(any jar with different size or checksum), these new classes are loaded at runtime, also there is a catch with dowloading the classes, they need to be cached somewhere as explained [in its wiki](https://github.com/edvin/fxlauncher/wiki/Optional-Cache-Directory).

In this library this is used with a class loader called URLClassLoader which according to [this document](https://www.oracle.com/technical-resources/articles/javase/classloaders.html) was previously used to load classes in Java applets(little java programs that used to be common in web pages some years ago).
...
Per JEP 395, Records are classes that act as transparent carriers for immutable data. As a result, we cannot stop a record from exposing its member fields. On the other hand, Lombok allows us to customize the names, access levels, and return types of the getters.
